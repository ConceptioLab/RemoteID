...
/* bluez and GDBus paths and interfaces(advertising-api.txt) */
...
static GVariant *get_service_data(void)
{
  ...
  /* remember the size limit of 31 bytes in total (i.e.
     including uuids) */
  if (adv_data.curr_uuid == 0)
    adv_data.service_data = "Stan";
  else if (adv_data.curr_uuid == 1)
    adv_data.service_data = "Satan";
  else if (adv_data.curr_uuid == 2)
    adv_data.service_data = "stan@sat.an";
...
int main(int argc, char **argv)
{
  ...
  if (g_dbus_connection_unregister_object(conn, reg_id1) == FALSE || g_dbus_connection_unregister_object(conn, reg_id2) == FALSE)
  {
    g_print("failed\n");
  }
  else
